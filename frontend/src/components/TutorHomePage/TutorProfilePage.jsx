...
const TutorProfilePage = () => {
  ...
  useEffect(() => {
    async function fetchUserData() {
      try {
        ...
        // Initialize formData with all relevant fields from userData
        setFormData({
          name: userData.name || '',
          email: userData.email || '',
          bio: userData.bio || '',
          subjects: userData.subjects ? userData.subjects.join(', ') : '',
          preferredTimes: userData.preferredTimes || '',
          // Add other fields as needed (e.g., experience, education)
        });
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // Convert subjects back to an array before updating
      const updatedData = { ...formData, subjects: formData.subjects.split(',').map(s => s.trim()) };
      ...
      <Container
        ...
        >
          <form onSubmit={handleSubmit}>
            ...
            <FormLabel htmlFor="subjects">Areas of Expertise:</FormLabel>
